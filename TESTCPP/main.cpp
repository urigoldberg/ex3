...
int main() {
	...
	NumOfSiftExtracted = createAllDB(NumOfSiftExtracted, &RGB_DB, &SIFTS_DB,
			dir, picName, HowManypic, sufName, nBin, nSift);
	...
	while (Query[0] != '#') {
		...
		SPPoint** SIFTS_Query = spGetSiftDescriptors(Query, 0, nSift,
				&howManySiftsQuery);
		if (SIFTS_Query == NULL) {
			terminate(RGB_DB, SIFTS_DB, NumOfSiftExtracted, HowManypic, false);
		...
		//Print 5 closest
		int test1 = print_5_LocalFeatures(SIFTS_Query, SIFTS_DB,
				NumOfSiftExtracted, howManySiftsQuery, HowManypic);
